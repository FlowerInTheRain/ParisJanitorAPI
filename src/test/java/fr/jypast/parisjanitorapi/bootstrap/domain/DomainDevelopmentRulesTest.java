package fr.jypast.parisjanitorapi.bootstrap.domain;
...
import static fr.jypast.parisjanitorapi.bootstrap.PackagesAndLayers.*;

@AnalyzeClasses(
    packages = "fr.barlords.parisjanitorapi.domain",
    cacheMode = FOREVER,
...
public class DomainDevelopmentRulesTest {
  ...
  public static final ArchRule DOMAIN_DEVELOPMENT_RULE =
      classes()
          ...
          .andShould().onlyAccessClassesThat().resideInAnyPackage(allowedPackages)
          .andShould().onlyBeAccessed().byClassesThat().resideInAnyPackage(allowedAccessors)
          .andShould().onlyHaveDependentClassesThat().resideInAnyPackage(allowedAccessors);
}
